...
data = {
    ...
    "state": np.random.choice(
        ["CA", "NY", "TX", "FL", "IL", "PA", "OH", "GA", "NC", "MI"], n_merchants
    ),
    ...
    "monthly_transaction_volume": np.random.lognormal(7.0, 1.2, n_merchants).astype(
        int
    ),
    "merchant_rating": np.random.choice(
        [3.0, 3.5, 4.0, 4.5, 5.0], n_merchants, p=[0.05, 0.15, 0.30, 0.35, 0.15]
    ),
    ...
    "merchant_status": np.random.choice(
        ["Active", "Inactive", "Suspended"], n_merchants, p=[0.90, 0.08, 0.02]
    ),
    ...
    locations.append(
        {
            ...
            "risk_level": np.random.choice(
                ["Low", "Medium", "High", "Critical"], p=[0.30, 0.40, 0.25, 0.05]
            ),
...
selected_customer_ids = np.random.choice(
    df_customers["customer_id"].values, n_transactions
)
selected_merchant_ids = np.random.choice(
    df_merchants["merchant_id"].values, n_transactions
)
...
transaction_data = {
    ...
    "time_since_last_transaction_hours": np.random.exponential(
        24, n_transactions
    ).round(2),
