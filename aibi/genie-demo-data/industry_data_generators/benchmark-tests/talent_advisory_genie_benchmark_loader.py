...
BENCHMARKS = [
    ...
    {"question": "In 2025, how many hires and exits did each business unit have, and what was the annual attrition rate?", "difficulty": "MEDIUM",
     "sql": "SELECT business_unit, SUM(hires) AS hires_2025, SUM(exits) AS exits_2025, ROUND(SUM(exits) * 100.0 / NULLIF(SUM(start_headcount), 0), 2) AS attrition_rate_pct FROM `{catalog}`.`{schema}`.`mart_workforce_planning` WHERE snapshot_year = 2025 GROUP BY business_unit ORDER BY attrition_rate_pct DESC"},
