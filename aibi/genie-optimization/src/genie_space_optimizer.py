...
Analyzes and optimizes Databricks Genie Spaces through:
1. Space visualization - Display serialized space JSON by section
2. LLM-powered checklist analysis - Evaluate configuration against best practices
3. Benchmark testing - Run Genie conversations and compare SQL results

Requirements:
    - CAN EDIT permission on the Genie Space
    - Access to a SQL warehouse (for benchmark execution)
    - Access to Foundation Model API endpoint

Parameters:
    - space_id: The Genie Space ID to analyze
    - warehouse_id: Optional SQL warehouse ID for benchmark execution
    - llm_endpoint: LLM endpoint for analysis (default: databricks-claude-sonnet-4-5)
"""
...
from databricks.sdk.service.dashboards import GenieMessage, GenieStartConversationMessageRequest
...
# Checklist content for LLM analysis (from space-checklist.md)
...
class HTMLRenderer:
    ...
    def render_summary_table(results: list[dict]) -> str:
        ...
        header_color = "#22c55e" if pass_rate >= 80 else "#f59e0b" if pass_rate >= 50 else "#ef4444"
...
class LLMAnalyzer:
    ...
    def _analyze_section(self, section_name: str, section_data: Any, checklist: str) -> dict:
...
Return your analysis as JSON with this structure:
{
    ...
    "summary": "overall assessment in 1-2 sentences"
        ...
        section_json = json.dumps(section_data, indent=2, default=str) if section_data else "null"
...
class GenieSpaceClient:
    ...
    def ask_question(self, space_id: str, question: str, max_wait_seconds: int = 120) -> GenieConversationResult:
...
class BenchmarkRunner:
    ...
    def __init__(self, genie_client: GenieSpaceClient, warehouse_id: Optional[str] = None):
    ...
    def _execute_sql(self, sql: str) -> tuple[Optional[int], Optional[str]]:
        ...
        try:
            ...
            if response.status and response.status.state == StatementState.SUCCEEDED:
                ...
                elif response.manifest and response.manifest.total_row_count is not None:
    ...
    def _compare_results(
        ...
        if expected_rows == generated_rows:
            return True, f"Row counts match: {expected_rows}", expected_rows, generated_rows
        else:
            return (
                False,
    ...
    def run_benchmark(self, space_id: str, benchmark: dict) -> BenchmarkResult:
        ...
        if not self.warehouse_id:
            ...
            summary = "SQL generated (not executed - no warehouse)" if passed else "No SQL generated"
            return BenchmarkResult(
                question=question,
                expected_sql=expected_sql,
                generated_sql=generated_sql,
    ...
    def run_all_benchmarks(self, space_id: str, benchmarks: list[dict]) -> list[BenchmarkResult]:
        """Run all benchmark tests."""
        results = []
        for i, benchmark in enumerate(benchmarks):
            print(f"Running benchmark {i + 1}/{len(benchmarks)}: {benchmark.get('question', '')[:50]}...")
...
# COMMAND ----------
# SECTION 1: Fetch and Display Serialized Space

print("=" * 80)
print("SECTION 1: Genie Space Configuration")
print("=" * 80)
...
# COMMAND ----------
# SECTION 2: LLM Analysis - Data Sources

print("=" * 80)
print("SECTION 2: LLM Checklist Analysis")
print("=" * 80)
...
# COMMAND ----------
# SECTION 2: LLM Analysis - Instructions
...
# COMMAND ----------
# SECTION 2: LLM Analysis - Benchmarks
...
# COMMAND ----------
# SECTION 2: Combined Analysis Summary
...
print("=" * 80)
print("SECTION 3: Benchmark Testing")
print("=" * 80)
...
# COMMAND ----------
# SECTION 3: Benchmark Results Summary
