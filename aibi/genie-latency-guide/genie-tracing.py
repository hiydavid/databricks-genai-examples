...
# MAGIC - **`agent`** — the [Agent mode API](https://docs.databricks.com/aws/en/genie-agents/api) (Beta). Starts a response, then polls conversation items (`reasoning` → `function_call` → `function_call_output` → ... → final report). Agent mode runs multi-step research, so expect minutes, not seconds.
# MAGIC
# MAGIC ## Note on timing
# MAGIC
# MAGIC **Chat mode:** step durations (`+Xs`) are computed from server-side `last_updated_timestamp` values, so they reflect actual Genie processing time. **Agent mode:** the items API exposes no per-item server timestamps, so step durations are poll-observed (client-side) deltas, quantized by the polling interval. In both modes the elapsed time (`t=`) is client-side and the polling interval defaults to 1 second. (GET polls do not count toward QPM limits.)
# MAGIC
# MAGIC ## Prerequisites
# MAGIC
...
# MAGIC 4. For `agent` mode only: Unity Catalog enabled, partner-powered AI features enabled, and the **Genie Agents Agent Mode API** preview turned on for the workspace (see [requirements](https://docs.databricks.com/aws/en/genie-agents/api#requirements))
...
# MAGIC 3. **Timing** — items carry no server-side timestamps, so step durations below are poll-observed deltas (quantized by the poll interval), unlike chat mode's server-side timestamps.

# COMMAND ----------


def _read_first_sse_event(resp, max_lines: int = 100):
    ...
    for i, line in enumerate(resp.iter_lines(decode_unicode=True)):
        ...
        if line.startswith("event:"):
            ...
            try:
                return event, json.loads(line[len("data:") :].strip())
            except ValueError:
                return event, None
    return event, None
...
def ask_genie_agent_with_full_trace(
    ...
    with mlflow.start_span(name="genie_query") as root:
        ...
        root.set_attributes({"mode": "agent"})
        ...
        with mlflow.start_span(name="1_create_response") as span:
            ...
            if resp.status_code != 200:
                ...
                if "FEATURE_DISABLED" in body:
                    error += (
                        " — enable the 'Genie Agents Agent Mode API' preview"
                        " for this workspace (see Previews in the workspace admin menu)"
                    )
                _set_span_error(span, resp.status_code, body)
                _finish_result(
                    result, start, error=error, final_status="CREATE_RESPONSE_FAILED"
        ...
        if top_status == "completed":
            with mlflow.start_span(name="3_extract_results") as extract_span:
                        ...
                        for chunk in item.get("content") or []:
                            ...
                            if meta and meta.get("columns"):
                                tables.append(
                                    {
                                        "columns": [
                                            c.get("name") for c in meta["columns"]
                                        ],
                ...
                if tables:
                    ...
                    result["sample"] = [
                        dict(zip(last_table["columns"], r))
                        for r in last_table["preview_rows"][:3]
                    ]
                ...
                if log_full_responses:
                    outputs["sql_queries"] = sql_queries
